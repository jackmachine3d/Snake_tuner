...
def first_position():
    ...
    target_position = [random.randrange(1, 40) * 10, random.randrange(1, 40) * 10]
    score = 3
    return snake_start, snake_position, target_position, score
...
def target_display(target_position, display):
    pygame.draw.rect(display, (0, 255, 0), pygame.Rect(target_position[0], target_position[1], 10, 10))


def apple_distance_from_snake(target_position, snake_position):
    return np.linalg.norm(np.array(target_position) - np.array(snake_position[0]))


def generate_snake(snake_start, snake_position, target_position, button_direction, score):
    ...
    if snake_start == target_position:
        target_position, score = collision_with_apple(target_position, score)
...
def collision_with_apple(target_position, score):
    target_position = [random.randrange(1, 40) * 10, random.randrange(1, 40) * 10]
    score += 1
...
def angle_with_apple(snake_position, target_position):
    apple_direction_vector = np.array(target_position) - np.array(snake_position[0])
...
def play_game(snake_start, snake_position, target_position, button_direction, score, display, clock):
    crashed = False
    while crashed is not True:
        ...
        target_display(target_position, display)
        display_snake(snake_position, display)

        snake_position, target_position, score = generate_snake(snake_start, snake_position, target_position,
                                                               button_direction, score)

        pygame.display.set_caption("Очки: " + str(score) )
